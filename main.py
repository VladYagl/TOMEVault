...
class VaultSoup:
    def __init__(self):
        self.soup = BeautifulSoup(open("temp.html", "r", encoding="utf-8").read(), 'html.parser')
        self.filters = {}
    ...
    def get_filter_values(self, filter_name):
        ...
        for op in self.soup.find('select', {'name': self.tag(filter_name)}).contents:
            if type(op) is bs4.element.Tag and op.get('data-official'):
                values[op.contents[0]] = op.get('value')
        return values

    def get_talents(self, talents_type, soup):
    ...
    def get_table(self, table_name, soup, as_list=None):
    ...
    def request(self, filters, min_level="", max_level="", winner=True,
        ...
        for page in range(10):
            ...
            req += '&tag_game[]=699172' # current game version tome-1.7.4
            if winner:
            ...
            req += '#'
            print(req)
            chars = BeautifulSoup(urllib.request.urlopen(req).read(), 'html.parser').find('div', {'id': 'characters'})
...
def plot_talents(talents_type, chars):
    ...
    for cat_name, cat in cats.items():
        print(cat_name, colored_perc(cat.taken, len(chars)))
        for talent, stat in cat.talents.items():
            print(f'\t{talent:25}', end='')
            for i in stat:
                print(' ' + colored_perc(i, cat.taken, True, [20, 40, 100]) + f'{i:2}' + colorama.Style.RESET_ALL, end='')
            print()
        print()


# chars = VaultSoup().request({'difficulty': ['insane', 'madness'], 'class': ['brawler']}, winner=True)
chars = VaultSoup().request({'difficulty': ['insane', 'madness'], 'class': ['brawler']}, min_level=12, max_level=25, winner=False)
print('\n\n\n\n\t------< CLASS TALENTS >------\n')
